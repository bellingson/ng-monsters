...
angular.module('monsterApp').factory('monsterService', function($q) {
	...
	function get(monsterId) {
		...
		return $q(function(resolve, reject) {
			...
			var m = _.filter(_monsters,function(m) { return m.id == monsterId; });
			var monster = m.length ? m[0] : null;
			resolve(monster);
		});
	...
	function update(monster) {
		...
		return $q(function(resolve, reject) {
			...
			_.remove(monsters, function(m) { return m.id == monster.id } );
			_monsters.push(monster);
	...
	function deleteMonster(monster) {
		...
		return $q(function(resolve, reject) {
			...
			_.remove(_monsters, function(m) { return m.id == monster.id; } );			
			resolve()
		});	
	...
	function monsters() {
		...
		return $q(function(resolve, reject) {
			var monsters = _monsters.sort(function(m1,m2) { return new Date(m2.modified) - new Date(m1.modified); });
			resolve(monsters);
		});
	}
